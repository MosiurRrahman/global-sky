...
const VisaDetails = ({ data }) => {
    ...
    const [selectedCategory, setSelectedCategory] = useState("Student visa");
    const [categories, setCategories] = useState([]);
    const [countries, setCountries] = useState([]);
    const [nationalities, setNationalities] = useState([]);
    const [isActiveCountry, setIsActiveCountry] = useState(false);
    const [isActiveCategory, setIsActiveCategory] = useState(false);
    const [searchCountryQuery, setSearchCountryQuery] = useState(""); // Added for dynamic search

    // Nationality and residency states
    ...
    const [searchNationalityQuery, setSearchNationalityQuery] = useState(""); // Added for nationality search
    const [searchResidencyQuery, setSearchResidencyQuery] = useState(""); // Added for residency search
...
// Handle click outside for nationality dropdown
const handleClickOutsideNationality = (event) => {
    if (dropdownRefNationality.current && !dropdownRefNationality.current.contains(event.target)) {
        setIsActiveNationality(false);
    }
};

// Handle click outside for residency dropdown
const handleClickOutsideResidency = (event) => {
    if (dropdownRefResidency.current && !dropdownRefResidency.current.contains(event.target)) {
    ...
    // Fetch countries and automatically set categories for Dubai
    useEffect(() => {
        const fetchCountries = async () => {
            try {
                const response = await fetch(`${base_url + "api/page/home/search-bar"}`);
                const data = await response.json();
                setCountries(data.data.countries);
                setNationalities(data.data.countries);

                // Automatically set categories for Dubai
                const dubaiCountry = data.data.countries.find((country) => country.name === "Dubai");
                if (dubaiCountry) {
                    fetchCategoriesForCountry(dubaiCountry.id);
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        document.addEventListener("click", handleClickOutsideNationality);
        document.addEventListener("click", handleClickOutsideResidency);
        ...
        return () => {
            ...
            document.removeEventListener("click", handleClickOutsideNationality);
            document.removeEventListener("click", handleClickOutsideResidency);
        };
    }, []);
    // Fetch categories for the selected country (Dubai in this case)
    const fetchCategoriesForCountry = async (countryId) => {
        try {
            const response = await fetch(`${base_url}api/get/category/${countryId}`);
            const data = await response.json();
            setCategories(data.data);

            // Automatically set "Tourist Visa" as the selected category
            const studentVisaCategory = data.data.find((category) => category.name === "Tourist Visa");
            if (studentVisaCategory) {
                setSelectedCategory(studentVisaCategory.name);
            }
        } catch (error) {
    ...
    const handleSelectCountry = (country) => {
        ...
        setIsActiveCountry(false);
        fetchCategoriesForCountry(country.id);
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (selectedCountry && selectedCategory) {
            const queryUrl = `/visa-details?country=${encodeURIComponent(selectedCountry)}&category=${encodeURIComponent(selectedCategory)}&=nationality=${encodeURIComponent(selectedNationality)}&=residency=${encodeURIComponent(selectedResidency)}`;
    ...
    const filteredNationalities = useMemo(
        () => data.countries.filter((nationality) =>
            nationality.nationality.toLowerCase().startsWith(searchNationalityQuery.toLowerCase())
        ),
        [searchNationalityQuery, data.nationalities]
    );

    const filteredResidencies = useMemo(
        () => data.residencies.filter((residency) =>
            residency.name.toLowerCase().startsWith(searchResidencyQuery.toLowerCase())
        ),
        [searchResidencyQuery, data.residencies]
    );
                                                                ...
                                                                <ul className="option-list">
                                                                    {Array.isArray(filteredCountries) && filteredCountries.length > 0 ? (
                                                                        filteredCountries.map((country) => (
                                                                            <li
                                                                                key={country.id}
                                                                                className={`single-item ${selectedCountry === country.name ? "active" : ""}`} // Add active class
                                                                        ...
                                                                        <li className="single-item"><h6>No country available</h6></li>
                                                                    )}
                                                                </ul>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="col-xl-3 col-sm-6 d-flex justify-content-center">
                                                ...
                                                <div className="icon">
                                                    <svg width="23" height="23" viewBox="0 0 23 23"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path
                
.1587 14.7529ZM17.9398 9.89328C16.9796 9.88615 16.043 9.59489 15.248 9.05623C14.4531 8.51758 13.8355 7.75563 13.4729 6.86645C13.1104 5.97728 13.0193 5.00068 13.2109 4.05977C13.4026 3.11886 13.8686 2.25577 14.5501 1.57929C15.2316 0.902822 16.0981 0.443256 17.0404 0.258533C17.9827 0.073809 18.9586 0.172197 19.8451 0.541294C20.7316 0.91039 21.4889 1.53367 22.0217 2.33256C22.5544 3.13145 22.8388 4.07019 22.8388 5.03043C22.8323 6.32442 22.313 7.56304 21.3946 8.47465C20.4762 9.38626 19.2338 9.89643 17.9398 9.89328ZM17.9398 1.81043C17.3045 1.81756 16.6855 2.01245 16.1608 2.37058C15.636 2.7287 15.229 3.23405 14.9908 3.82302C14.7526 4.41199 14.694 5.05826 14.8223 5.68048C14.9506 6.3027 15.26 6.87306 15.7118 7.31978C16.1635 7.7665 16.7373 8.0696 17.3609 8.19095C17.9845 8.31229 18.6301 8.24645 19.2164 8.00171C19.8026 7.75697 20.3034 7.34426 20.6557 6.81555C21.0079 6.28684 21.1959 5.66574 21.1959 5.03043C21.1907 4.17147 20.8449 3.34967 20.2345 2.74537C19.624 2.14107 18.7987 1.80364 17.9398 1.80714V1.81043Z" />
                                                    </svg>
                                                </div>
                                                    <div className="searchbox-input">
                                                                ...
                                                                <ul className="option-list">
                                                                    {Array.isArray(categories) && categories.length > 0 ? (
                                                                        categories.map((category) => (
                                                                            <li
                                                                                key={category.id}
                                                                                className={`single-item ${selectedCategory === category.name ? "active" : ""}`} 
                                                                        ...
                                                                        <li className="single-item"><h6>No category available</h6></li>
                                                                    )}
                                                                </ul>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="col-xl-3 col-sm-6 d-flex justify-content-center">
                                                ...
                                                <div className="icon">
                                                    <svg width="23" height="23" viewBox="0 0 23 23"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                                            d="M18.2479 3.18226H16.7709V1.23888C16.7709 0.845507 16.599 0.49699 16.287 0.257587C15.9749 0.0180925 15.5937 -0.0577078 15.2138 0.0441046L3.46137 3.19313L3.46146 3.1934C3.32514 3.22986 3.22461 3.35385 3.22461 3.50167V22.6806C3.22461 22.7653 3.25827 22.8465 3.31817 22.9064C3.37808 22.9663 3.45933 23 3.54406 23H18.2479C19.0899 23 19.7749 22.315 19.7749 21.473V4.70922C19.7749 3.86725 19.0899 3.18226 18.2479 3.18226ZM15.3792 0.661186C15.5682 0.610576 15.7427 0.645259 15.898 0.764413C16.0532 0.883521 16.132 1.04315 16.132 1.23884V3.18222H5.97044L15.3792 0.661186ZM3.8635 3.82116H4.53599V22.3611H3.8635V3.82116ZM19.136 21.473C19.136 21.9627 18.7376 22.3611 18.2479 22.3611H5.17488V3.82116H18.2479C18.7376 3.82116 19.136 4.21955 19.136 4.70922V21.473H19.136ZM12.1555 14.0779C14.5775 14.0779 16.548 12.1074 16.548 9.68539C16.548 7.26339 14.5775 5.2929 12.1555 5.2929C9.73344 5.2929 7.76295 7.26339 7.76295 9.68539C7.76295 12.1074 9.73344 14.0779 12.1555 14.0779ZM10.1319 10.0048C10.149 10.5296 10.208 11.0365 10.3054 11.5044C9.94911 11.6702 9.61679 11.8833 9.31738 12.1378C8.8132 11.5551 8.48522 10.8166 8.41645 10.0048H10.132H10.1319ZM13.
17.0806 9.06965 17.047 9.15437 17.047ZM10.0579 16.0977C9.97314 16.0977 9.89189 16.0641 9.83198 16.0042C9.77207 15.9443 9.73841 15.863 9.73841 15.7783C9.73841 15.6936 9.77207 15.6123 9.83198 15.5524C9.89189 15.4925 9.97314 15.4589 10.0579 15.4589H14.253C14.3377 15.4589 14.419 15.4925 14.4789 15.5524C14.5388 15.6123 14.5724 15.6936 14.5724 15.7783C14.5724 15.863 14.5388 15.9443 14.4789 16.0042C14.419 16.0641 14.3377 16.0977 14.253 16.0977H10.0579ZM16.9363 18.4707H7.37455C7.28983 18.4707 7.20857 18.5044 7.14867 18.5643C7.08876 18.6242 7.0551 18.7055 7.0551 18.7902V20.57C7.0551 20.6547 7.08876 20.7359 7.14867 20.7958C7.20857 20.8558 7.28983 20.8894 7.37455 20.8894H16.9362C17.021 20.8894 17.1022 20.8558 17.1621 20.7958C17.222 20.7359 17.2557 20.6547 17.2557 20.57V18.7902C17.2557 18.7055 17.222 18.6242 17.1621 18.5643C17.1022 18.5044 17.021 18.4707 16.9362 18.4707H16.9363ZM16.6168 20.2505H7.694V19.1096H16.6168V20.2505H16.6168Z" />
                                                    </svg>
                                                </div>
                                                    <div className="searchbox-input">
                                                        <label>Nationality</label>
                                                        <div className="custom-select-dropdown" ref={dropdownRefNationality}>
                                                            <div className="select-input" onClick={() => setIsActiveNationality(!isActiveNationality)}>
                                                            ...
                                                            <div className={`${isActiveNationality ? "active" : ""} custom-select-wrap`}>
                                                                <div className="custom-select-search-area">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        value={searchNationalityQuery}
                                                                        onChange={(e) => setSearchNationalityQuery(e.target.value)}
                                                                    />
                                                                </div>
                                                                <ul className="option-list">
                                                                    {Array.isArray(filteredNationalities) && filteredNationalities.length > 0 ? (
                                                                        filteredNationalities.map((nationality, index) => (
                                                                            <li
                                                                                key={index}
                                                                                className={`single-item ${selectedNationality === nationality.nationality ? "active" : ""}`} // Add active class
                                                                                onClick={() => handleSelectNationality(nationality.nationality)}
                                                                            >
                                                                                <h6>{nationality.nationality}</h6>
                                                                                <img src={base_url + nationality.flag}  onError={(e) => e.target.src = "/path/to/placeholder-image.png"} alt="Nationality flag" />
                                                                            </li>
                                                                        ))
                                                                    ) : (
                                                                        <li className="single-item"><h6>No nationality available</h6></li>
                                                                    )}
                                                                </ul>
                                                            </div>
                                                        </div>
                                                ...
                                                <div className="icon">
                                                    ...
                                                    <div className="searchbox-input">
                                                        ...
                                                        <div className="custom-select-dropdown" ref={dropdownRefResidency}>
                                                            <div className="select-input" onClick={() => setIsActiveResidency(!isActiveResidency)}>
                                                                <input
                                                                    type="text"
                                                                    readOnly
                                                            ...
                                                            <div
                                                                className={`${isActiveResidency ? "active" : ""} custom-select-wrap`}
                                                            >
                                                                <div className="custom-select-search-area">
                                                                    <i className="bx bx-search" />
                                                                    <input
                                                                        type="text"
                                                                        placeholder="Type Residency"
                                                                        value={searchResidencyQuery}
                                                                        onChange={(e) => setSearchResidencyQuery(e.target.value)}
                                                                    />
                                                                </div>
                                                                <ul className="option-list">
                                                                    {Array.isArray(filteredResidencies) && filteredResidencies.length > 0 ? (
                                                                        filteredResidencies.map((residency, index) => (
                                                                            <li
                                                                                key={index}
                                                                                className={`single-item ${selectedResidency === residency.name ? "active" : ""}`} 
